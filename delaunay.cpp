...
FPoint operator^(FLine q,FLine w){
/// source: https://ru.wikipedia.org/wiki/%D0%9F%D0%B5%D1%80%D0%B5%D1%81%D0%B5%D1%87%D0%B5%D0%BD%D0%B8%D0%B5_%D0%BF%D1%80%D1%8F%D0%BC%D1%8B%D1%85
    auto x1=q.q.x;
    auto x2=q.w.x;
    auto x3=w.q.x;
    auto x4=w.w.x;
    ...
    auto y3=w.q.y;
    auto y4=w.w.y;
    return ((w.q-w.w)*(x1*y2-y1*x2)-(q.q-q.w)*(x3*y4-y3*x4))/((x1-x2)*(y3-y4)-(y1-y2)*(x3-x4));
}
...
auto IfRound(GlobalLine* l){
    ...
    auto t=
        __int128_t(
            (q->q[1]-q->q[0])/(q->q[2]-q->q[0])
        )*__int128_t(
            (w->q[1]-w->q[0])*(w->q[2]-w->q[0])
        )+__int128_t(
            (q->q[1]-q->q[0])*(q->q[2]-q->q[0])
        )*__int128_t(
            (w->q[1]-w->q[0])/(w->q[2]-w->q[0])
...
bool Flip(GlobalLine* l) {
    ...
    tie(
        ...
        decltype(q->q){q->q[2], q->q[0], w->q[0]},
        decltype(q->r){q->r[0], w->r[2], q->r[1]},
        decltype(q->l){q->l[0], w->l[2], q->l[1]},
        decltype(w->q){w->q[2], w->q[0], q->q[0]},
        decltype(w->r){w->r[0], q->r[2], w->r[1]},
        decltype(w->l){w->l[0], q->l[2], w->l[1]}
    );
#ifdef CHECK
...
#if __has_include("/Users/gera/c/test.py")
...
void Draw2(Line l, array<size_t, 3>& color, array<size_t, 3>& color1) {
    Draw1(Line{(l.q * BestInt(3) + l.w * BestInt(0)) / BestInt(3), (l.q * BestInt(2) + l.w * BestInt(1)) / BestInt(3)}, color);
    Draw1(Line{(l.q * BestInt(2) + l.w * BestInt(1)) / BestInt(3), (l.q * BestInt(1) + l.w * BestInt(2)) / BestInt(3)}, color);
    Draw1(Line{(l.q * BestInt(1) + l.w * BestInt(2)) / BestInt(3), (l.q * BestInt(0) + l.w * BestInt(3)) / BestInt(3)}, color);
}
...
int main() {
    ...
    long double yd=-390;
    size_t n = scan;
    // array<FLine,4> border({{{0,0},{xx,0}},{{xx,0},{xx,yy}},{{xx,yy},{0,yy}},{{0,yy},{0,0}}});
    // array<FLine,4> border({{{},{xx,0}},{{xx,0},{xx,yy}},{{xx,yy},{0,yy}},{{0,yy},{0,0}}});
    array<pair<FPoint,FPoint>,4> border({
        pair<FPoint,FPoint>{{xd,yd},{xu,yd}},
        pair<FPoint,FPoint>{{xu,yd},{xu,yu}},
        pair<FPoint,FPoint>{{xu,yu},{xd,yu}},
        pair<FPoint,FPoint>{{xd,yu},{xd,yd}},
    });
    // border[0]=Line{{xd,yd},{xu,yd}};
    // border[1]=Line{{xd,yd},{xu,yd}};
    // border[2]=Line{{xd,yd},{xu,yd}};
    // border[3]=Line{{xd,yd},{xu,yd}};
    vector<Point> a(n);
    ScanVector(a);
    auto o=a;
    sort(a.begin(),a.end());
    ...
    if (c==0){
        ...
        > j;
        for (auto&p:edges){
            s[p.first.q].push_back({&p,p.first.w});
            s[p.first.w].push_back({&p,p.first.q});
        }
        for (auto&d:s){
            ic("======================================================================================================================================================")
            auto&g=d.second;
            ...
            auto&k=j[d.first];
            assert(not g.empty());
            sort(g.begin()+1,g.end(),[&](auto&&q,auto&&w){
                auto z=Sign((d.first-g[0].second)/(q.second-d.first));
                auto x=Sign((d.first-g[0].second)/(w.second-d.first));
                if (z>0 and x<0){
                    // ic(q,w,false)
                    return false;
                }
                ...
                auto tmp= (q.second-d.first)/(w.second-d.first)>0;
                // ic(q,w,tmp)
                return tmp;
            });
            ...
            //     auto&z=g[q];
            //     auto&x=g[(q+1)%g.size()];
            //     assert((z.second-d.first)/(x.second-d.first)>0);
            // }
            #endif
            for (size_t q=0;q<g.size();++q){
                auto&z=g[q];
                auto&x=g[(q+1)%g.size()];
                Triangle*t=nullptr;
                if (z.first->second.first!=nullptr and z.first->second.first==x.first->second.first){
                    t=z.first->second.first;
                }
                if (z.first->second.second!=nullptr and z.first->second.second==x.first->second.first){
                    t=z.first->second.second;
                }
                if (z.first->second.first!=nullptr and z.first->second.first==x.first->second.second){
                    t=z.first->second.first;
                }
                if (z.first->second.second!=nullptr and z.first->second.second==x.first->second.second){
                    t=z.first->second.second;
                }
                // ic(t)
                if (t){
                    if ((z.second-d.first)/(x.second-d.first)<=0){
                        t=nullptr;
                    }
                ...
                // if (z.first){
                //     ic(z.first[0])
                //     if (z.first->second.first){
                //         ic(z.first->second.first[0])
                //     }
                //     if (z.first->second.second){
                //         ic(z.first->second.second[0])
                //     }
                // }
                // if (x.first){
                //     ic(x.first[0])
                //     if (x.first->second.first){
                //         ic(x.first->second.first[0])
                //     }
                //     if (x.first->second.second){
                //         ic(x.first->second.second[0])
                //     }
                // }
                if (t){
                    // ic(d.first)
            ...
            for (size_t q=0;q<h.size();++q){
                ...
                // ic(z,x)
                if (z!=z){
                    auto v=(c.second-d.first);
                    v={v.y,-v.x};
                    // v*=BestInt(10000000);
                    auto b=FPoint({static_cast<long double>(v.x),static_cast<long double>(v.y)});
                    assert((b!=FPoint{0,0}));
                    assert(x==x);
                    for (size_t q=0;;q=(q+1)%4){
                        if ((border[q].second-border[q].first)/(x+b-border[q].first)<=0){
                            // ic(border[q],x+b,(border[q].second-border[q].first),(x+b-border[q].first),(border[q].second-border[q].first)/(x+b-border[q].first))
                            break;
                        }
                        b*=2;
                    }
                    z=x+b;
                    // auto n=llroundl(atan2(b.y,b.x)/M_PI_4);
                    // ic(b,n)
                    // auto n1=static_cast<size_t>("\x03\x03\x02\x02\x01\x01\x00\x00\x03"[n+4]);
                    // auto n2=static_cast<size_t>("\x03\x02\x02\x01\x01\x00\x00\x03\x03"[n+4]);
                    // auto z1=border[n1]^FLine(x,x+b);
                    // auto z2=border[n2]^FLine(x,x+b);
                    // if ((z1-x).SqAbs()>(z2-x).SqAbs()){
                    //     z=z2;
                    // }else{
                    //     z=z1;
                    // }
                    // auto t=Line{x,x+FPoint({static_cast<long double>(v.x),static_cast<long double>(v.y)})};
                    // z=x+FPoint({static_cast<long double>(v.x),static_cast<long double>(v.y)});
                    // ic(d.first)
                    // ic(z)
                    // ic(c)
                    // ic(x)
                ...
                if (x!=x){
                    ...
                    assert(z==z);
                    for (size_t q=0;;q=(q+1)%4){
                        if ((border[q].second-border[q].first)/(z+b-border[q].first)<=0){
                            // ic(border[q],z+b,(border[q].second-border[q].first),(z+b-border[q].first),(border[q].second-border[q].first)/(z+b-border[q].first))
                            break;
                        }
                        b*=2;
                    }
                    ...
                    // auto n1=static_cast<size_t>("\x03\x03\x02\x02\x01\x01\x00\x00\x03"[n+4]);
                    // auto n2=static_cast<size_t>("\x03\x02\x02\x01\x01\x00\x00\x03\x03"[n+4]);
                    // auto x1=border[n1]^FLine(z,z+b);
                    // auto x2=border[n2]^FLine(z,z+b);
                    // if ((x1-z).SqAbs()>(x2-z).SqAbs()){
                    //     x=x2;
                    // }else{
                    //     x=x1;
                    // }
                    // ic(z,b,z+b,x)
                    // x=z+FPoint({static_cast<long double>(v.x),static_cast<long double>(v.y)});
                    // ic(d.first)
                    // ic(z)
                    // ic(c)
                    // ic(x)
        ...
        for (auto&cc:nodes){
            Point ccp{(llroundl)(cc.x),(llroundl)(cc.y)};
            // ic(ccp)
            Draw(ccp);
        }

        for (auto&p:border){
            Draw(Line{{llroundl(p.first.x),llroundl(p.first.y)},{llroundl(p.second.x),llroundl(p.second.y)}});
        }

        // array<Point,Point,Point,POin
            ...
            for (auto&o:p.second){
                ...
                while (1){
                    ...
                    for (size_t q=0;q<4;++q){
                        if ((border[q].second-border[q].first)/(o.first-border[q].first)<-0.0000001){
                            c+=100+q;
                            break;
                        }
                    }
                    for (size_t q=0;q<4;++q){
                        if ((border[q].second-border[q].first)/(o.second-border[q].first)<-0.0000001){
                            c+=200+q;
                            break;
                        }
                    }
                    ...
                    if (c>=200){
                        o.second=FLine(o.first,o.second)^FLine{border[c-200].first,border[c-200].second};
                        if (Abs(o.second.x-xd)<0.0000001){
                            o.second.x=xd;
                        }
                        if (Abs(o.second.x-xu)<0.0000001){
                            o.second.x=xu;
                        }
                        if (Abs(o.second.y-yd)<0.0000001){
                            o.second.y=yd;
                        }
                        if (Abs(o.second.y-yu)<0.0000001){
                            o.second.y=yu;
                        }
                    }else
                    if (c>=100){
                        o.first=FLine(o.first,o.second)^FLine{border[c-100].first,border[c-100].second};
                        if (Abs(o.first.x-xd)<0.0000001){
                            o.first.x=xd;
                        }
                        if (Abs(o.first.x-xu)<0.0000001){
                            o.first.x=xu;
                        }
                        if (Abs(o.first.y-yd)<0.0000001){
                            o.first.y=yd;
                        }
                        if (Abs(o.first.y-yu)<0.0000001){
                            o.first.y=yu;
                        }
                    }
                    // ic(o)
        ...
        for (auto&p:j){
            ...
            for (size_t q=0;q<o.size();++q){
                ...
                auto x=o[(q+1)%o.size()];
                if (z.second!=x.first){
                    if (z.second.x==x.first.x and (x.first.x==xd or x.first.x==xu)){
                        o.insert(o.begin()+q+1,{
                            z.second,
                            x.first
                        });
                        continue;
                    }
                    if (z.second.y==x.first.y and (x.first.y==yd or x.first.y==yu)){
                        o.insert(o.begin()+q+1,{
                            z.second,
                            x.first
                        });
        ...
        for (auto&p:j){
            ...
            for (auto o:p.second){
                // ic(o)
                if (o==o){
                    Line i={{llroundl(o.first.x),llroundl(o.first.y)},{llroundl(o.second.x),llroundl(o.second.y)}};
                    Draw(i);
                    auto u=FPoint{p.first.x+0.,p.first.y+0.};
                    o.first=o.first*BestFloat(0.8)+u*BestFloat(0.2);
                    o.second=o.second*BestFloat(0.8)+u*BestFloat(0.2);
                    i={{llroundl(o.first.x),llroundl(o.first.y)},{llroundl(o.second.x),llroundl(o.second.y)}};
                    Draw(i);
                }
            }
        }
        ...
        for (auto&p:a){
            ...
            // sort(d.second.begin(),d.second.end(),[&](auto&&z,auto&&x){
            //     auto f=[&](auto&f){
            //         auto z=f->first.q;
            //         auto x=f->first.w;
            //         #ifdef CHECK
            //         assert((z==d.first) ^ (x==d.first));
            //         #endif
            //         if (x==d.first){
            //             swap(z,x);
            //         }
            //         x=x-z;
            //         return atan2l((long double)x.x,(long double)x.y);
            //     };
            //     return f(z)<f(x);
            // });
            // for (size_t q=0;q<d.second.size();++q){
            //     auto&z=d.second[q];
            //     auto&x=d.second[(q+1)%d.second.size()];
            //     Triangle*t=nullptr;
            //     if (z->second.first!=nullptr and z->second.first==x->second.first){
            //         t=z->second.first;
            //     }
            //     if (z->second.second!=nullptr and z->second.second==x->second.first){
            //         t=z->second.second;
            //     }
            //     if (z->second.first!=nullptr and z->second.first==x->second.second){
            //         t=z->second.first;
            //     }
            //     if (z->second.second!=nullptr and z->second.second==x->second.second){
            //         t=z->second.second;
            //     }
            //     if (t){
