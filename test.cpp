...
struct CheckIter {
    ...
    static_assert(
        std::is_same_v<decltype(std::declval<iter>() - std::declval<iter>()), typename traits::difference_type>);
...
void testWithSize() {
    ...
    assert((less_simple.size() == size_t(size)) &&
           std::all_of(less_simple.begin(), less_simple.end(), [&](const auto& item) { return item.data == value; }));
    Deque<DefaultConstructible> default_constructor(size);
    assert(std::all_of(default_constructor.begin(), default_constructor.end(),
                       [](const auto& item) { return item.data == DefaultConstructible::default_data; }));
}
...
/*
    void testStrongGuarantee() {
        ...
        auto is_intact = [&] {
            return d.size() == size && std::all_of(d.begin(), d.end(), [initial_data](const auto& item) {return
...
int main() {
    ...
    static_assert(
        !std::is_same_v<std::deque<TestsByMesyarik::VerySpecialType>, Deque<TestsByMesyarik::VerySpecialType>>,
        "You cannot use std::deque, cheater!");
    static_assert(
        !std::is_base_of_v<std::deque<TestsByMesyarik::VerySpecialType>, Deque<TestsByMesyarik::VerySpecialType>>,
        "You cannot use std::deque, cheater!");
