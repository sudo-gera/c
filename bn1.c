...
int bn_cmp(bn const *q, bn const *e){
	...
	for (size_t c=(qs>es?qs:es)-1;;--c){
		size_t a=qs>c?q->vect[c]:0;
		size_t s=es>c?e->vect[c]:0;
		if (a!=s){
...
static void bn_M_add_to(bn *q, bn const *e){
	...
	if (q->size and qs>q->size){
		...
		q->vect=(uint32_t*)calloc(q->vect,1,qs*sizeof(uint32_t));
		*(uint64_t*)(q->vect)=tmp;
		qv=q->vect;
		q->size=qs;
...
void bn_root_to(bn*q,int __e){
	...
	if (tmp>2){
		...
		_e->size=0;
		_e->sign=0;
		_e->vect=(uint32_t*)(uint64_t)(1LL<<32);
	}
	bn*_c=bn_new();
	bn*_d=bn_new();	
	bn*_f=bn_new();
...
void bn_init_string_radix(bn*q,const char*e,int t){
	size_t l=strlen(e);
	q->size=l/"\x00\x00\x20\x14\x10\x0d\x0c\x0b\x0a\x0a\x09\x09\x08\x08\x08\x08\x08\x07\x07\x07\x07\x07\x07\x07\x06\x06\x06\x06\x06\x06\x06\x06\x06\x06\x06\x06\x06"
		[t]+1;
	q->sign=0;
	if (q->vect){
		free(q->vect);
...
const char* bn_to_string(const bn*q,int e){
	...
	bn_abs(r);
	bn*t=bn_new();
	char*a=(char*)calloc(sizeof(char),1+(q->size+1)*"\x00\x00\x20\x15\x10\x0e\x0d\x0c\x0b\x0b\x0a\x0a\x09\x09\x09\x09\x08\x08\x08\x08\x08\x08\x08\x08\x07\x07\x07\x07\x07\x07\x07\x07\x07\x07\x07\x07\x07"[_t]);
	size_t s=0;
	while (r->sign){
		uint64_t b=0;
		uint64_t t=0;
