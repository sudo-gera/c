...
struct pll{
    llu first=0,second=0;
    // operator std::pair<llu,llu>()const{
    //     return {first,second};
    // }
    auto&out(std::ostream&o){
        o<<"<"<<first<<", "<<second<<">";
        return o;
    }
};

auto&operator<<(std::ostream&o,pll r){
    return r.out(o);
}


pll operator*(pll q,llu w){
    return {q.first*w,q.second*w};
}

pll operator*(llu q,pll w){
    return {w.first*q,w.second*q};
}

pll operator+(pll q,pll w){
    return {q.first+w.first,q.second+w.second};
}

bool operator==(pll q,pll w){
    return q.first==w.first and q.second==w.second;
}
...
#ifdef assert_m
enable_print(pll,val.first,val.second);
	...
	struct el{
		...
		void make()const{
			if (to_mul!=1){
                ...
                const_cast<T&>(first)={0,0};
                const_cast<T&>(last)={0,0};
                const_cast<T&>(v)={0,0};
				if (z){
					const_cast<long&>(z->to_mul)*=to_mul;
					const_cast<T&>(z->to_add)=z->to_add*to_mul;
				}
				if (x){
					const_cast<long&>(x->to_mul)*=to_mul;
					const_cast<T&>(x->to_add)=x->to_add*to_mul;
				}
				if (to_mul==0){
			...
			if (to_add!=pll{0,0}){
				...
				const_cast<T&>(first)=first+to_add;
				const_cast<T&>(last)=last+to_add;
				const_cast<T&>(v)=v+to_add;
				if (z){
					const_cast<T&>(z->to_add)=z->to_add+to_add;
				}
				if (x){
					const_cast<T&>(x->to_add)=x->to_add+to_add;
				}
				const_cast<T&>(to_add)={0,0};
			}
		...
		void update()const{
			if (z){
				...
				// }else{
					const_cast<T&>(first)=z->first*z->to_mul+z->to_add;
				// }
			}else{
				const_cast<T&>(first)=v;
			}
			if (x){
				...
				// }else{
					const_cast<T&>(last)=x->last*x->to_mul+x->to_add;
				// }
			}else{
				const_cast<T&>(last)=v;
			}
...
public:
    ...
    template<typename Y,typename COMP>
	iter upper_bound(const Y&d,COMP comp){
		// if (is_forward()){
			iter t;
			auto q=e_get();
			if (!q){
	...
	void add(T v){
		if (e_get()){
			const_cast<T&>(_e->to_add)=_e->to_add+v;
		}
	}
	void mul(long v){
		if (e_get()){
...
int main(){
    ...
    {
        vector<long> s(a.begin(),a.end());
        sort(s.begin(),s.end());
        for (auto&w:a){
            w=lower_bound(s.begin(),s.end(),w)-s.begin();
        }
    }
    ...
    for (auto&w:a){
        d.push_back({s[w].first,s[w].second});
        d.back().second+=1;
        llu r=s.upper_bound(d.back(),[&](auto z,auto x){return z.second<x.second;})-s.begin();
        // ic(a)
        // ic(s)
        // ic(d)
        // ic()
        {
            auto x=s.cut_left(r);
            auto z=x.cut_left(w+1);
            // ic(z,x,s)
            x.mul(0);
            x.add({0LLU+(&w-&a[0]),d.back().second});
            x.add_left(z);
            s.add_left(x);
        }

        // for (llu q=w+1;q<r;++q){
        //     s.update(q,{0LLU+(&w-&a[0]),d.back().second});
        // }
        // ic(a)
        // ic(s)
        // ic(d)
