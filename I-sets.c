...
int main(){
	...
	for (uint64_t w=0;w<q;++w){
		...
		if (str[0]=='?' and str[1]==0){
			long s=getint();
			if (a[s]==0 or a[s]->len==0){
				write(-1);
			}else{
				long*tmp=0;
				for (uint64_t w=0;w<len(a[s]->data);w++){
					for (uint64_t e=0;e<len(a[s]->data[w]);e++){
						append(tmp,a[s]->data[w][e].key);
					}
				}
				qsort(tmp,len(tmp),sizeof(tmp[0]),(cmp_f_t)cmp1);
				for (uint64_t w=0;w<len(tmp);w++){
					write(tmp[w]);
				}
				del(tmp);
			}
