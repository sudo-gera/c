...
private:
	struct el{
		...
		void update()const{
			...
			if (z and x){
				...
				const_cast<long&>(min)=std::min({v , z->min+z->to_add , x->min+x->to_add});
			}else
			if (z){
				// long s_x_forward=1;
				// long s_x_backward=1;
				...
				const_cast<long&>(min)=std::min({v , z->min+z->to_add});
			}else
			if (x){
				// long s_z_forward=1;
				// long s_z_backward=1;
				...
				const_cast<long&>(min)=std::min({v ,  x->min+x->to_add});
			}else{
				const_cast<long&>(s)=1;
				const_cast<long&>(d)=1;
	...
	static void check(const el*s){
		...
		if (s->z and s->x){
			...
			assert(s->min==std::min({s->z->min+s->z->to_add,s->x->min+s->x->to_add,s->v}));
		...
		if (s->z){
			assert(s->z->s+1==s->s);
			assert(s->z->d+1==s->d);
			assert(s->min==std::min({s->z->min+s->z->to_add,s->v}));
		...
		if (s->x){
			assert(s->x->s+1==s->s);
			assert(s->x->d+1==s->d);
			assert(s->min==std::min({s->x->min+s->x->to_add,s->v}));
