...
ldouble zero=0.000012345;

ldouble pi=3.141592653589793;

struct Point{
	ldouble x,y;
	Point(){}
	Point(ldouble q,ldouble w):x(q),y(w){}
};
...
public:
	...
	Line(ldouble k,ldouble b){
		*this=Line({0,b},{1,b+k});
	}
	Line(Point a,ldouble k){
...
vect operator*(vect q,ldouble e);

struct vect{
	ldouble x=0,y=0;
	vect(){};
	vect(ldouble _x,ldouble _y):
		x(_x),
		y(_y)
	{}
	...
	vect(Point q,Point e,ldouble r,ldouble t);
	vect(Line q){
		*this=vect(q.a,q.s);
	}
};
...
vect::vect(Point q,Point e,ldouble r,ldouble t){
	*this=vect(q,e)*r+vect(q,e).perp()*t;
}

ldouble angle(vect q,vect e){
	return q.angle(e);
}

ldouble angle(Point q,Point w,Point e){
...
vect operator*(ldouble q,vect e){
	return {q*e.x,q*e.y};
}
...
bool operator==(Line q,Line e){
	auto a=vect(q)/vect(q).abs();
	auto s=vect(e)/vect(e).abs();
...
class Shape{
public:
	virtual ldouble perimeter()=0;
	virtual ldouble area()=0;
	...
	virtual void scale(Point center, ldouble coefficient)=0;
	virtual ~Shape(){}
};

Point rotate(Point q,Point e,ldouble r){
	return e+vect(e,q).rotate(r);
}

Point scale(Point q,Point e,ldouble r){
	return e+vect(e,q)*r;
}
...
public:
	ldouble a,b,c,e;
	Point f1,f2;
	Ellipse(Point _f1,Point _f2,ldouble d):f1(_f1),f2(_f2){
		a=d/2;
	...
	ldouble perimeter(){
		ldouble res=0;
		ldouble dt=0.0000004;
		ldouble tmp;
		for (ldouble t=0;t<pi/2;t+=dt){
			tmp=cos(t)*e;
	...
	void rotate(Point center, ldouble angle){
	...
	void scale(Point center, ldouble coefficient){
...
public:
	...
	Rectangle(Point q,Point e,ldouble f){
		if (f>=1){
...
bool simPolygon(T q,Y e,bool no_scale=0){
	...
	size_t mw=0;
	ldouble m=vect(q.vertices[0],q.vertices[1]).abs();
...
int main(){
	ldouble q,w,e,r,t,y,u,i;
	std::cin>>q>>w>>e>>r>>t>>y>>u>>i;
	auto a=Point(q,w);
	auto s=Point(e,r);
	auto d=Point(t,y);
	auto f=Point(u,i);
	auto g=Line(a,s);
	auto h=Line(d,f);
	if (vect(g).to_line(d)==h){
		if (Polygon(a,s).containsPoint(d) or Polygon(a,s).containsPoint(f) or Polygon(d,f).containsPoint(a) or Polygon(d,f).containsPoint(s)){
			std::cout<<"YES"<<std::endl;
		}else{
			std::cout<<"NO"<<std::endl;
		}
	}else{
		auto j=intersect(g,h);
		if (Polygon(a,s).containsPoint(j) and Polygon(d,f).containsPoint(j)){
			std::cout<<"YES"<<std::endl;
		}else{
			std::cout<<"NO"<<std::endl;
		}
