...
async def open_connection(host, port, *, interface, limit=2**16):
    loop = asyncio.get_running_loop()
    ...
    for family, socktype, proto, _, address in infos:
        ...
        try:
            ...
            print(f"{interface}: connecting to {address}")

            await loop.sock_connect(sock, address)

            print(f"{interface}: TCP connected")

            reader = asyncio.StreamReader(limit=limit)
            protocol = asyncio.StreamReaderProtocol(reader)

            # IMPORTANT:
            # This is synchronous and takes ownership of `sock`.
            transport = loop._make_socket_transport(
                sock,
                protocol,
                None,
            )
...
async def copy(args: main_args, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
    while (data := await reader.read(2**16)):
        logging.info('data: '+repr(data))
        writer.write(data)
        await writer.drain()

async def on_connection(args: main_args, reader_accepted: asyncio.StreamReader, writer_accepted: asyncio.StreamWriter) -> None:
    try:
        logging.info('accepted')
        reader_connected, writer_connected = await open_connection(args.connect_host, args.connect_port, interface=args.interface)
        try:
            logging.info('connected')
            await gather(
                copy(args, reader_accepted, writer_connected),
                copy(args, reader_connected, writer_accepted),
            )
        finally:
            writer_connected.close()
            await writer_connected.wait_closed()
    finally:
        writer_accepted.close()
        await writer_accepted.wait_closed()

async def main(args: main_args) -> None:
    set_log_level(args.log_level)
    async with await asyncio.start_server(partial(on_connection, args), args.listen_host, args.listen_port) as server:
