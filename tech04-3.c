...
{
    int64_t size_summ = 0;
    char *str_buffer = sizeof("link_to")+calloc(sizeof(char) , PATH_MAX+ sizeof("link_to"));

    memcpy(str_buffer-sizeof("link_to"),"link_to_",sizeof("link_to"));
    ...
    while (fgets(str_buffer, PATH_MAX, stdin) != NULL)
    {
    	...
    	if (lstat(str_buffer, &f_stat) == 0) {
    		if(S_ISREG(f_stat.st_mode)) {
                symlink(str_buffer,str_buffer-sizeof("link_to"));
    		}else
    		if(S_ISLNK(f_stat.st_mode)) {
                char* buffer=realpath(str_buffer,NULL);
                printf("%s\n",buffer);
    		}
    	}
    }

    free(str_buffer-sizeof("link_to"));
