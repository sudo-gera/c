...
int testGeometry(){
	...
	if(oss.str()!="1000110"){
		std::cerr<<"Test 15 failed.(containspoint)\n";
		return 1;
	}
	// std::cerr<<"TestsLog: Test 15 completed,now just scale shapes(without any checks)\n";

	for(size_t i=0;i<shapes.size();++i){
		shapes[i]->scale(Point(5,5),0.5);
	}
	// std::cerr<<"TestsLog:Now starting testing ellipse(tests16.0-16.2)\n";
	//Ellipsetesting
	cf5=Ellipse(f,c,5);
	//std::cerr<<"TestsLog:NowtheremustbeonlyoneEllipse,withfocuses(3;-1),(6;1)anddist5\n";
	{
		double c=sqrt(13)/2;
		double a=5./2;
		double e=c/a;
		...
		if(!equals(cf5.eccentricity(),e)){
			std::cerr<<"Test16.0failed.(ellipseeccentricity)\n";
			return 1;
		}
		if(!equals(cf5.perimeter(),per)){
			std::cerr<<"Test16.1failed.(ellipseperimeter)\n";
			return 1;
		}
		if(!equals(cf5.area(),ar)){
			std::cerr<<"Test16.2failed.(ellipsearea)\n";
			return 1;
		}
	}
	...
	abd=Triangle(d,b,a);
	{
		...
		//eulercirclecenterliesinthemiddleofsegmentbetweenorthocenterandcircumcenter
		ok=equals(distance(orc,eulerCircle.center()),distance(circ,eulerCircle.center()));
		if(!ok){
			std::cerr<<"Test17.1failed.(nine-pointcirclecenter)\n";
			return 1;
		}
		...
		if(!ok){
			std::cerr<<"Test 17.2 failed.(radius of the nine-point circle is half of the circum circle radius)\n";
			return 1;
		}
		//Описаннаяокружностьестьобразокружностидевятиточек
		//относительногомотетиисцентромвортоцентреикоэффициентом2
		Circle anotherCircle=eulerCircle;
		anotherCircle.scale(orc,2);
		ok=circumcircle==anotherCircle;
		if(!ok){
			std::cerr<<"Test17.3failed.(homothety of nine-points circle wrt orthocenter with k=2 gives circum circle)\n";
			return 1;
		}

		//прямаяЭйлерапроходитчерезцентроидицентрокружностидевятиточек
		ok=Line(eulerCircle.center(),abd.centroid())==eulerLine;

		if(!ok){
			std::cerr<<"Test17.4failed.(centroidandnine-pointscirclecenterlieonEulerline)\n";
			return 1;
		}
	}
