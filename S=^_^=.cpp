...
vector<int> Lcp(const T &s, const vector<int> &suf_array) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (ind[i_cur] != 0) {
            ...
            while (i_cur + last < static_cast<int>(s.size()) && i_prev + last < static_cast<int>(s.size()) &&
                   s[i_cur + last] == s[i_prev + last]) {
                ++last;
            }
...
int main() {
    ...
    string t = scan;
    string f = s + " " + t;
    vector<int> fa(f.begin(), f.end());
    auto fs = suffix_array::SuffixArray(fa, 128);
    auto fl = suffix_array::Lcp(fa, fs);
    size_t w = 0;
    size_t k = scan;
    --k;
    for (size_t q = 1; q < fs.size(); ++q) {
        w = min<size_t>(w, static_cast<size_t>(fl[q - 1]));
        if (fl[q - 1] and static_cast<size_t>(fs[q - 1]) < s.size() and static_cast<size_t>(fs[q]) > s.size()) {
            auto p = static_cast<size_t>(fl[q - 1]) - w;
            if (k < p) {
                w += k;
                cout << string(s.begin() + fs[q - 1], s.begin() + fs[q - 1] + static_cast<ssize_t>(w) + 1) << endl;
                return 0;
            }
            w += p;
            k -= p;
            // auto t=min(k,fl[q-1]-w);
            // w+=t;
            // k-=t;
            // if (t!=fl[q-1]-w){
            //     cout<<string(s.begin()+fs[q-1],s.begin()+fs[q-1]+w+1)<<endl;
            //     return 0;
            // }
            // for (;w<fl[q-1];++w){
            //     cout<<string(s.begin()+fs[q-1],s.begin()+fs[q-1]+w+1)<<endl;
        ...
        if (fl[q - 1] and static_cast<size_t>(fs[q - 1]) > s.size() and static_cast<size_t>(fs[q]) < s.size()) {
            ...
            if (k < p) {
                w += k;
                cout << string(s.begin() + fs[q], s.begin() + fs[q] + static_cast<ssize_t>(w) + 1) << endl;
                return 0;
            }
            w += p;
            k -= p;
            ...
            // k-=t;
            // if (t!=fl[q-1]-w){
            //     cout<<string(s.begin()+fs[q],s.begin()+fs[q]+w+1)<<endl;
            //     return 0;
            // }
            // // for (;w<fl[q-1];++w){
            //     cout<<string(s.begin()+fs[q],s.begin()+fs[q]+w+1)<<endl;
