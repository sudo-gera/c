...
async def socks5h(args: main_args, accepted_reader: asyncio.StreamReader, accepted_writer: asyncio.StreamWriter, params: list[str]) -> None:

        proxy_host, proxy_port_str, forward_host, forward_port_str = params
        ...
        try:
            ...
            try:
                ip = ipaddress.ip_address(forward_host)
                addr = ip.packed
                if ip.version == 4:
                ...
                addr = forward_host.encode()
                addr = len(addr).to_bytes(1, 'big') + addr
            
            port = forward_port.to_bytes(2, 'big')
...
async def on_client(args: main_args, accepted_reader: asyncio.StreamReader, accepted_writer: asyncio.StreamWriter) -> None:
    try:
        ...
        protocol, *params = line.decode().split()

        logging.info(f"request {protocol = !r} {params = !r}")

        await {
            handler.__name__: handler
            for handler in protocol_handlers
        }[protocol](args, accepted_reader, accepted_writer, params)
