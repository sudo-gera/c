...
template <typename t0 = sized<0>, typename t1 = sized<0>, typename t2 = sized<0>, typename t3 = sized<0>,
          typename t4 = sized<0>, typename t5 = sized<0>, typename t6 = sized<0>, typename t7 = sized<0>>
// better than tuple
struct t {
    t0 v0;
    t1 v1;
...
template <typename... T>
auto t2tuple(t<T...> r) {
    return apply([&](auto... a) { return tuple(*(a)...); },
                 clear_tuple(tuple<>(), tuple{&r.v0, &r.v1, &r.v2, &r.v3, &r.v4, &r.v5, &r.v6, &r.v7}));
}
...
bool operator<(t<R...> r, t<Y...> y) {
    return t2tuple(r) < t2tuple(y);
}
template <typename... R, typename... Y>
bool operator>(t<R...> r, t<Y...> y) {
    return t2tuple(r) > t2tuple(y);
...
#define assign(f)                                 \
    assign_s {                                    \
        [&](auto q, auto w) { return (f)(q, w); } \
    }

///////////////////////////////////////////////////end of lib

auto prefix_fun(auto&& s) {
    vector<llu> p(s.size());
    for (llu q = 1; q < s.size(); ++q) {
        llu k = p[q - 1];
...
int main() {
    string s, p;
    cin >> s >> p;
    auto r = prefix_fun(p + " " + s);
    for (llu q = 0; q < r.size(); ++q) {
        if (r[q] >= p.size()) {
            cout << q - p.size() * 2 << "\n";
        }
    }
